...
class cifar_cnn(model_base):
    ...
    def decl_model(self):
        ...
        c1 = self.decl_conv2d_layer("c1",x1,[3,3,3,32],[32])
        p1 = self.decl_max_pool("max_pool1",c1,padding='SAME')
        
        c2 = self.decl_conv2d_layer("c2",p1,[3,3,32,64],[64])
        p2 = self.decl_max_pool("max_pool2",c2,padding='SAME')
        
        c3 = self.decl_conv2d_layer("c3",p2,[3,3,64,128],[128])
        p3 = self.decl_max_pool("max_pool3",c3,padding='SAME')
      
        flat = tf.reshape(p3,[-1,4 * 4 * 128])
        dense = self.decl_full_conn_layer("dense",flat,[4 * 4 * 128,256],[256])
